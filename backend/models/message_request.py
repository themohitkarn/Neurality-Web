...
class MessageRequest(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint("sender_id", "receiver_id", name="uq_message_requests_sender_receiver"),
    )

    sender = db.relationship(
        "User",
        foreign_keys=[sender_id],
        backref=db.backref("sent_message_requests", lazy="dynamic", cascade="all, delete-orphan"),
    )
    receiver = db.relationship(
        "User",
        foreign_keys=[receiver_id],
        backref=db.backref("received_message_requests", lazy="dynamic", cascade="all, delete-orphan"),
    )
